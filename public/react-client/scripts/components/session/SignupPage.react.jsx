...
var SignupPage = React.createClass({
  ...
  _onSubmit: function(e) {
    ...
    if (email.length == 0) {
      this.setState({ errors: ['Email is mandatory']});
      return;
    }
    var password = this.refs.password.getDOMNode().value;
    if (password.length == 0) {
      this.setState({ errors: ['Password is mandatory']});
      return;
    }
        ...
        <div className="row">
          <div className="card card--login small-10 medium-6 large-4 columns small-centered">
            <form onSubmit={this._onSubmit}>
              ...
              <button type="submit" className="btn btn-primary card--login__submit">Signup</button>
            </form>
          </div>
        </div>
      </div>
